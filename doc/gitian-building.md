...
- [Building Hyperstake](#building-sppcoin)
...
git clone https://github.com/sppcoin/sppcoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sppcoin repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the sppcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sppcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/sppcoin/sppcoin
...
For example:
```bash
URL=https://github.com/robvanmieghem/sppcoin.git
COMMIT=master
./bin/gbuild --commit sppcoin=${COMMIT} --url sppcoin=${URL} ../sppcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sppcoin=${COMMIT} --url sppcoin=${URL} ../sppcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sppcoin=${COMMIT} --url sppcoin=${URL} ../sppcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sppcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sppcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sppcoin-osx-build.assert
```
...
[sppcoin/gitian.sigs](https://github.com/sppcoin/gitian.sigs/) repository, or if that's not possible create a pull
