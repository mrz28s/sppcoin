    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s SPP %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
						...
						// change SPP-Coin to get rid of automatic generation of change address and instead send change back
...
{
    //presstab SPP-Coin: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    // presstab SPP-Coin - Initialize as static and don't update the set on every run of CreateCoinStake() in order to lighten resource use
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
    {
		...
		//SPP-Coin now iterates each utxo inside of CheckStakeKernelHash()
		if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNewTime, nHashDrift, false, hashProofOfStake))
		{
			...
			//presstab SPP-Coin - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
			uint64 nCoinAge;
			CTxDB txdb("r");
			...
			uint64 nTotalSize = pcoin.first->vout[pcoin.second].nValue + GetProofOfStakeReward(nCoinAge, nAge);
				
			//presstab SPP-Coin - if MultiSend is set to send in coinstake we will add our outputs here (values asigned further down)
	...
	if(fCombineDust) //presstab SPP-Coin - this combination code iterates through all of your outputs on successful coinstake, so its useful to have user be able to choose whether this is necessary
	{
		BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
		{
	...
	uint64 nReward;
    {
		...
		nReward = GetProofOfStakeReward(nCoinAge, nAge);
        nCredit += nReward;
    }
