...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	// stake age of full weight: -1
unsigned int nStakeTargetSpacing = 120;			// 2 minute block spacing

int64 nChainStartTime = 1470375004;
...
const string strMessageMagic = "SPP-Coin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nAge)
{
    int64 nRewardCoinYear;
    unsigned int nTimeYear = 31536000; //seconds per year

	if(nAge < nTimeYear)
        nRewardCoinYear = 5 * CENT;
    else if(nAge < nTimeYear * 2)
        nRewardCoinYear = 7 * CENT;
    else if (nAge < nTimeYear * 3)
        nRewardCoinYear = 9 * CENT;
    else if(nAge < nTimeYear * 4)
        nRewardCoinYear = 11 * CENT;
    else
        nRewardCoinYear = 13 * CENT;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            uint64 nCoinAge;
			unsigned int nAge = 0;
            if (!GetCoinAge(txdb, nCoinAge, nAge))
                return error("ConnectInputs() : %s unable to get coin age for coinstake", GetHash().ToString().substr(0,10).c_str());
            int64 nStakeReward = GetValueOut() - nValueIn;
            if (nStakeReward > GetProofOfStakeReward(nCoinAge, nAge))
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SPP-Coin
    bool fStrictPayToScriptHash = true; // Always active in SPP-Coin
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge, unsigned int& nAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if(nAge == 0)
            nAge = nTime - txPrev.nTime;
        else if(nTime - txPrev.nTime < nAge) //use the lowest age in the transaction to determine the age
            nAge = nTime - txPrev.nTime;
...
bool CBlock::GetCoinAge(uint64& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        uint64 nTxCoinAge;
		unsigned int nAge = 0;
        if (tx.GetCoinAge(txdb, nTxCoinAge, nAge))
            nCoinAge += nTxCoinAge;
        else
            return false;
    }
...
bool CBlock::AcceptBlock()
{
	...
	/*	SPP-Coin is not using a checkpoint server
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock, std::string& strErr)
{
	...
	/* SPP-Coin is not using checkpoint server
	...
	// presstab SPP-Coin: enable of disable staking based on block difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SPP-Coin", CClientUIInterface::OK | CClientUIInterface::ICON_ETRKLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "8/4/2016 - CoinDesk - Bitcoin Price Regains Ground as Traders Brace for Bitfinex News";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1470375004;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1709291;
        if(fTestNet)
		{
			block.nTime = 1470375004;
			block.nNonce = 0;
        ...
        assert(block.hashMerkleRoot == uint256("4320b9b28cc9d211775bd83dd169fcaf9051b1865a59b43436962d73d4965016"));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
		...
		/* SPP-Coin does not use checkpoint server
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake && !pwalletMain->fDisableStake)  // attempt to find a coinstake && make sure settings allow PoS (presstab SPP-Coin)
    {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	if(pblock->nTime > GetAdjustedTime() + GetClockDrift(1))
		return error("BitcoinMiner : block too far in the future");
	
	if(pblock->nTime < GetAdjustedTime() - GetClockDrift(1))
		return error("BitcoinMiner : block too far in the past");
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked()  ||  (fProofOfStake && !fMintableCoins))
        {
        ...
        if(fProofOfStake && mapHashedBlocks.count(nBestHeight)) //search our map of hashed blocks, see if bestblock has been hashed yet
        {
