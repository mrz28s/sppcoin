...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
			...
			// Potential Stake
			double nPotentialStake = GetProofOfStakeReward(out.tx->vout[out.i].nValue * nAge / COIN / (24 * 60 * 60), nAge);
			...
			double nEstimateTime = 120 * nNetworkWeight / nBlockWeight; // 120 seconds is block target
